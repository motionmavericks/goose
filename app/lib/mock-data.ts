export const mockData = {
    conferenceName: "SportNXT 2025",
    sidebarCategories: [
      {
        title: "Overview",
        items: [
          { name: "All Photos", icon: "ImageIcon" },
          { name: "Highlights", icon: "Star" },
          { name: "Speakers", icon: "Users" },
        ],
      },
      {
        title: "Schedule",
        items: [
          { name: "Day 1 - Main Stage", icon: "Calendar" },
          { name: "Day 2 - Breakouts", icon: "Calendar" },
          { name: "Day 3 - Site Visits", icon: "Calendar" },
        ],
      },
      {
        title: "Session Types",
        items: [
          { name: "Keynotes", icon: "Users" },
          { name: "Panels", icon: "Users" },
          { name: "Workshops", icon: "Users" },
        ],
      },
    ],
    tabItems: [
      { value: "day1", label: "Main Stage" },
      { value: "day2", label: "Breakouts" },
      { value: "day3", label: "Site Visits" },
    ],
    mainContent: [
      {
        title: "Main Stage Sessions",
        description: "Keynote addresses and feature panels on critical issues in sport.",
        items: [
          {
            title: "Global Keynote Address",
            description: "Opening Session",
            image: "/placeholder.svg?height=400&width=400",
            time: "9:00 AM",
          },
          {
            title: "The State of Sport",
            description: "Macro Trends Panel",
            image: "/placeholder.svg?height=400&width=400",
            time: "10:30 AM",
          },
          {
            title: "Capital Investment Trends",
            description: "Financial Insights",
            image: "/placeholder.svg?height=400&width=400",
            time: "11:45 AM",
          },
          {
            title: "Women's Sport Evolution",
            description: "Gender Equity in Sports",
            image: "/placeholder.svg?height=400&width=400",
            time: "2:00 PM",
          },
        ],
      },
      {
        title: "Breakout Streams",
        description: "Focused sessions on key areas in the sports industry.",
        items: [
          {
            title: "Commercial & Revenue",
            description: "Monetization Strategies",
            image: "/placeholder.svg?height=400&width=400",
            time: "9:30 AM",
          },
          {
            title: "Brands & Marketing",
            description: "Engaging Audiences",
            image: "/placeholder.svg?height=400&width=400",
            time: "11:00 AM",
          },
          {
            title: "Fan Experience",
            description: "Enhancing Engagement",
            image: "/placeholder.svg?height=400&width=400",
            time: "2:30 PM",
          },
          {
            title: "Emerging Technology",
            description: "Innovation in Sports",
            image: "/placeholder.svg?height=400&width=400",
            time: "4:00 PM",
          },
        ],
      },
      {
        title: "Interactive Workshops",
        description: "Hands-on sessions exploring various topics in sports.",
        items: [
          {
            title: "Sustainability in Sports",
            description: "Green Initiatives",
            image: "/placeholder.svg?height=400&width=400",
            time: "10:00 AM",
          },
          {
            title: "Diversity & Inclusion",
            description: "Building Inclusive Sports",
            image: "/placeholder.svg?height=400&width=400",
            time: "1:30 PM",
          },
          {
            title: "Ethics & Integrity",
            description: "Maintaining Sport Values",
            image: "/placeholder.svg?height=400&width=400",
            time: "3:30 PM",
          },
        ],
      },
    ],
    mainStageSessions: [
      {
        id: 1,
        title: "Senate Press Gallery",
        description: "A view of the Senate press gallery",
        image: "/senate-press.jpg",
        time: "10:00 AM"
      }
    ],
    breakoutStreams: [
      {
        id: 1,
        title: "Treasurer Media",
        description: "The Treasurer and Minister for Finance talking to the media",
        image: "/treasurer-media.jpg",
        time: "11:00 AM"
      }
    ],
    interactiveWorkshops: [
      {
        id: 1,
        title: "Workshop 1",
        description: "Interactive session",
        image: "/workshop1.jpg",
        time: "2:00 PM"
      }
    ]
  }
  
  